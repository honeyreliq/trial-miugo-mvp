...
{
    public class EnableMarketingEmailsTests : TestBase
    {
        [Test]
        public void ShouldRequireValidPatient()
        {
            var command = new EnableMarketingEmailsCommand() { PatientId = Guid.NewGuid() };
        ...
        {
            var command = new EnableMarketingEmailsCommand() { PatientId = Guid.Empty };
        ...
        public async Task ShouldUpdatePatient()
        {
            ...
            var command = new EnableMarketingEmailsCommand() { PatientId = p.Id };
            await Testing.SendAsync(command);

            var patient = await Testing.FindAsync<Patient>(p.Id);
            patient.AllowMarketingEmails.Should().BeTrue();
