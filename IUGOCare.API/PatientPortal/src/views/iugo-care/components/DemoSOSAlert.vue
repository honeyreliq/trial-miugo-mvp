        ...
        <v-card-title class="headline">
          <v-row class="pb-4 mx-auto" align="center">
            ...
            <v-col cols="8">
              ...
              <v-row class="font-weight-bold">SOS</v-row>
              <v-row class="text-subtitle-1">{{sosWorkspace.alert.date}} @ {{sosWorkspace.alert.time}}</v-row>
            </v-col>
          </v-row>
        </v-card-title>
        <div class="pt-4 alert-card">
          <v-card-actions class="d-flex justify-space-around">
            <BaseButton color="primary" to="/iugo-care/workspace/sos" @click="clickButtonHandler(respondHandler)">
              Respond
            </BaseButton>
...
export default Vue.extend({
  ...
  methods: {
    ...
    setAlertData(): void {
      ...
      this.setSOSAlert(alertModel);
    },
    keyboardHandler(event: KeyboardEvent): void {
      const KEY = 'S';
      // COMMAND: command {ctrl} + option {alt} + S
      // Macintosh
