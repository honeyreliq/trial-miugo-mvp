<template>
  <div>
    <v-row v-for="measurement in a1cMeasurements" :key="measurement.code">
      <v-col cols="12">
...
<script lang="ts">
  ...
  export default Vue.extend({
    ...
    methods: {
      getA1cMeasurements(): IBasicMeasurement[] {
        return [
          {
            code: 'a1c',
            unit: '%',
            value: null as number,
            label: this.$t('a1c').toString(),
            rules: this.getA1cRules,
          },
        ];
      },
      ...
      saveObservation(request: IObservationRequest): void {
        ...
        this.a1cMeasurements.forEach((measurementItem) => {
          if (measurementItem &&
        ...
        obClient.createManualObservation(command).then(
          () => {
            this.getA1cMeasurements().forEach((measurementItem) => measurementItem.value = null);
      ...
      clearFields(): void {
        this.a1cMeasurements = this.getA1cMeasurements();
      },
    },
    computed: {
      getA1cRules(): ValidationFn[] {
        return [ exists, isDecimal(2), isBetween(1, 100) ];
      },
    },
    created() {
      this.a1cMeasurements = this.getA1cMeasurements();
