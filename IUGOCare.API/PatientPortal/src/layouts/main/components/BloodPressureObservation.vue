<template>
  <div>
    <v-row v-for="measurement in bloodPressureMeasurements" :key="measurement.code">
      <v-col cols="12">
...
<script lang="ts">
  ...
  import {ValidationFn, exists, isBetween, isDecimal} from '../../../validation';
  ...
  export default Vue.extend({
    ...
    methods: {
      getBloodPressureMeasurements(): IBasicMeasurement[] {
        return [
          {
            code: 'systolic',
            unit: 'mmHg',
            value: null as number,
            label: this.$t('systolic').toString(),
            rules: this.getSystolicRules,
          },
          {
            code: 'diastolic',
            unit: 'mmHg',
            value: null as number,
            label: this.$t('diastolic').toString(),
            rules: this.getDiastolicRules,
          },
          {
            code: 'heart-rate',
            unit: 'bpm',
      ...
      saveObservation(request: IObservationRequest): void {
        ...
        this.bloodPressureMeasurements.forEach((measurementItem) => {
          if (measurementItem &&
        ...
        obClient.createManualObservation(command).then(
          () => {
            this.getBloodPressureMeasurements().forEach((measurementItem) => measurementItem.value = null);
      ...
      clearFields(): void {
        this.bloodPressureMeasurements = this.getBloodPressureMeasurements();
      },
    },
    computed: {
      getSystolicRules(): ValidationFn[] {
        return [ exists, isDecimal(2), isBetween(35, 245) ];
      },
      getDiastolicRules(): ValidationFn[] {
        return [ exists, isDecimal(2), isBetween(35, 245) ];
      },
      getHeartRateRules(): ValidationFn[] {
        return [ isDecimal(2), isBetween(35, 500) ];
      },
    },
    created() {
      this.bloodPressureMeasurements = this.getBloodPressureMeasurements();
