<template>
  <div>
    <v-row v-for="measurement in oxygenSaturationMeasurements" :key="measurement.code">
      <v-col cols="12">
...
<script lang="ts">
  ...
  export default Vue.extend({
    name: 'OxygenSaturationObservation',
    data: () => ({
    ...
    methods: {
      getOxygenSaturationMeasurements(): IBasicMeasurement[] {
        return [
          {
            code: 'oxygen-saturation',
            unit: 'SpO2',
            value: null as number,
            label: this.$t('oxygenSaturation').toString(),
            rules: this.getOxygenSaturationRules,
          },
          {
            code: 'heart-rate',
            unit: 'bpm',
      ...
      saveObservation(request: IObservationRequest): void {
        ...
        this.oxygenSaturationMeasurements.forEach((measurementItem) => {
          if (measurementItem &&
        ...
        obClient.createManualObservation(command).then(
          () => {
            this.oxygenSaturationMeasurements.forEach((measurementItem) => measurementItem.value = null);
      ...
      clearFields(): void {
        this.oxygenSaturationMeasurements = this.getOxygenSaturationMeasurements();
      },
    },
    computed: {
      getOxygenSaturationRules(): ValidationFn[] {
        return [ exists, isDecimal(2), isBetween(50, 100) ];
      },
