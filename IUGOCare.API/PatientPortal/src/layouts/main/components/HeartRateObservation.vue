<template>
  <div>
    <v-row v-for="measurement in heartRateMeasurements" :key="measurement.code">
      <v-col cols="12">
...
<script lang="ts">
  ...
  export default Vue.extend({
    ...
    methods: {
      getHeartRateMeasurements(): IBasicMeasurement[] {
        return [
          {
            code: 'heart-rate',
            unit: 'bpm',
      ...
      saveObservation(request: IObservationRequest): void {
        ...
        this.heartRateMeasurements.forEach((measurementItem) => {
          if (measurementItem &&
      ...
      clearFields(): void {
        this.heartRateMeasurements = this.getHeartRateMeasurements();
      },
    },
    computed: {
      getHeartRateRules(): ValidationFn[] {
        return [ exists, isDecimal(2), isBetween(35, 500) ];
      },
    },
    created() {
      this.heartRateMeasurements = this.getHeartRateMeasurements();
